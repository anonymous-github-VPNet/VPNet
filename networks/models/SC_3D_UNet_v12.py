...
from models.Completion import Completion_v7_b3_selection_weight3
...
def conv7x7x7(in_planes, out_planes):
    return nn.Sequential(
        nn.Conv3d(in_planes, out_planes, 3, 1, padding=3, dilation=3,bias=False),
        nn.ReLU(),
        nn.Conv3d(in_planes, out_planes, 3, 1, padding=2, dilation=2,bias=False),
        nn.ReLU(),
        nn.Conv3d(in_planes, out_planes, 3, 1, padding=1, dilation=1,bias=False),
        nn.ReLU()
    )

def conv5x5x5(in_planes, out_planes):
    return nn.Sequential(
        nn.Conv3d(in_planes, out_planes, 3, 1, padding=2, dilation=2,bias=False),
        nn.ReLU(),
        nn.Conv3d(in_planes, out_planes, 3, 1, padding=1, dilation=1,bias=False),
...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        ############################  Completion  ######################################
        self.logits_ = conv3x3(4 * init_size, 2 * init_size, indice_key="logit_")
        self.logits_act = nn.LeakyReLU()
        self.logits_bn = nn.BatchNorm1d(2 * init_size)


        self.pre_com = Completion_v7_b3_selection_weight3(2*init_size, 2*init_size, spatial_shape=self.spatial_shape, bias=False, noise_channels=4)
    ...
    def forward(self, batch_dict, stat='train'):
        ...
        ###########   Completion  #################
        logits_ = self.logits_(up0e)
        logits_ = logits_.replace_feature(self.logits_act(logits_.features))
        logits_ = logits_.replace_feature(self.logits_bn(logits_.features))
      
        logits_ = logits_.dense()
        pre_completion, new_bxyz_0, new_bxyz_1, new_bxyz_2 = self.pre_com(logits_)

    
        return y, down1c, down2c, down3c, pre_completion, new_bxyz_0, new_bxyz_1, new_bxyz_2
        # return down1c, down2c, down3c, pre_completion, new_bxyz_0, new_bxyz_1, new_bxyz_2
